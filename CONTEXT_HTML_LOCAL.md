...
3. **앵커 에셋 로컬화**: `<a href>`가 `image_map.csv`에 있는 외부 URL을 가리키면 로컬 상대경로로 치환. CDN 이미지 다운로드 링크, GDrive 이미지 링크 등이 대상. `clean_url()`로 정규화 후 매칭.
4. **내부 링크 로컬화**: `slug_map` 기반 파일 간 상대경로. `/tag/{slug}/` → `{category}/index.html`. 블로그 루트 → `index.html`.
5. **홈 로고 리라이트**: `<a class="site-nav-logo">` → `index.html`
6. **YouTube iframe 반응형**: `width`/`height` 속성 제거 → `style="width:100%; aspect-ratio:16/9;"` 적용.
7. **JS 제거**: 모든 `<script>` 제거, `<noscript>` 유지.
...
`stale_html_local.csv` — MD와 동일 패턴. `HtmlLocalizer`가 unmapped URL을 수집, `run_html_local` 실행 시 자동 refresh 판정.
