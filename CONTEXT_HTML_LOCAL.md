...
`run_html_local()` 끝에서 호출. 블로그 서버에서 태그 목록 페이지를 fetch하여 로컬 목록 페이지 생성.

**동작 방식**:
1. 각 태그 페이지(`/tag/{slug}/`)의 1페이지를 fetch → `<link rel="next">`를 따라 전체 페이지 순회
2. 모든 `<article class="post-card">` 수집 후 1페이지 레이아웃에 합침
3. `HtmlLocalizer` 적용 (이미지·CSS·링크 로컬화, JS 제거)
4. `html_local/{category}/index.html`로 저장

**홈페이지**: 전체 포스트 목록은 `html_local/index_all.html`로 저장. 기본 `index.html`은 `_process_post`에서 블로그 홈페이지(첫 페이지 25개)를 직접 저장.

**생성 파일**:
- `html_local/{category}/index.html` (6개 카테고리)
- `html_local/index_all.html` (전체 목록)
- `html_local/index.html` (`_process_post`에서 생성)

**매 실행 시 서버 fresh fetch**. done 파일로 추적하지 않음 (파생 산출물).
